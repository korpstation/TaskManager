...
export default function DemoProfileScreen({ showToast }) {
  ...
  React.useEffect(() => {
    const load = async () => {
      if (!isDemoUser(username)) return;
      const lists = await getListsMock(userId);
      setAllLists(lists || []);
    };
    load();
  }, [username, userId]);
  ...
  const handleExportAsJSON = async () => {
    try {
      ...
      const fileBaseName = `${username}-demo-lists-${formatDateForFilename()}.json`;
      ...
      if (Platform.OS === "web") {
        ...
        showToast && showToast("Demo lists exported as JSON (downloaded)");
        return;
      }
      ...
      if (Platform.OS === "android") {
        ...
        try {
          ...
          showToast && showToast("Demo JSON saved to selected folder");
          return;
        } catch (e) {
          showToast &&
      ...
      if (Platform.OS === "ios") {
        ...
        try {
          await Sharing.shareAsync(uriToShare, {
            ...
            dialogTitle: "Export Demo Lists (JSON)",
          });
        ...
        showToast && showToast("Demo JSON ready to save in Files (iOS)");
        return;
      }
    } catch (e) {
      showToast &&
        showToast(e?.message || "Error exporting demo JSON", "error");
    }
  };
  ...
  const handleExportAsPDF = async () => {
    try {
      ...
      const fileBaseName = `${username}-demo-lists-${formatDateForFilename()}.pdf`;
      ...
      if (Platform.OS === "web") {
          ...
          for (const t of list.todos) {
            ...
            if (
              ...
              try {
                ...
                console.log("Error adding image to PDF (web demo):", err);
              }
            } else {
              y += 2;
            }
        ...
        showToast && showToast("Demo PDF downloaded");
        return;
      }
    ...
    `;
      ...
      if (Platform.OS === "android") {
        ...
        try {
          ...
          showToast && showToast("Demo PDF saved to selected folder (Android)");
          return;
        } catch (e) {
      ...
      if (Platform.OS === "ios") {
        ...
        try {
          await Sharing.shareAsync(uriToShare, {
            ...
            dialogTitle: "Export Demo Lists (PDF)",
          });
        ...
        showToast && showToast("Demo PDF ready to save in Files (iOS)");
        return;
      }
    } catch (e) {
      console.log("Demo PDF export error", e);
      showToast && showToast(e?.message || "Error exporting demo PDF", "error");
    }
  };
  ...
  const handleConfirmDeleteAllLists = async () => {
    try {
      ...
      if (!allLists || allLists.length === 0) {
        showToast && showToast("No demo lists to delete");
        return;
      }

      await deleteAllListsMock(userId);
      setAllLists([]);
      showToast && showToast("All demo lists deleted");
    } catch (e) {
      console.log("Delete all demo lists error", e);
      showToast &&
        showToast(e?.message || "Error deleting all demo lists", "error");
    }
  };
  ...
  const onDeleteAccount = async () => {
    try {
      ...
      showToast && showToast("Demo account deleted");
      logout();
    } catch (e) {
      showToast &&
        showToast(e?.message || "Error deleting demo account", "error");
    }
  };
  ...
  return (
    <View style={styles.container}>
            ...
            >
              <View style={styles.settingLeft}>
                ...
                <View style={styles.settingText}>
                  ...
                  <Text style={styles.settingDescription}>
                    Permanently delete your demo account
                  </Text>
                </View>
              </View>
            </TouchableOpacity>
      ...
      <ConfirmModal
        ...
        message="This will permanently delete your demo account and demo data."
        confirmLabel="Delete"
        cancelLabel="Cancel"
      ...
      <ConfirmModal
        ...
        title="Delete All Demo Lists?"
        message="This will permanently delete all your demo task lists and tasks."
